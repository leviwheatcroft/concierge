 ...
 * or whatever.
 *
 * note: `_.compactObject` method may introduce problems, falsy values are
 * not passed to `config` object. `cli` module allows you to set defaults,
 * if a default is falsy, it will not end up on `config` it will be `undefined`
 *
 */
init = function() {
  var cliOptions;
  ...
  cliOptions = getCommandLine();
  iniOptions = getIni(cliOptions.configFile);
  // cli options overwrite iniFile, which overwrite defaults
  _.extend(config, iniOptions, _.compactObject(cliOptions));
